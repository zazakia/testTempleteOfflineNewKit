...
const WsCustomerEntity: EntityDefinition<WsCustomer> = {
  name: 'ws_customer',
  ui: { label: 'WS Customer', labelPlural: 'WS Customers', icon: 'Droplets', routePath: 'water-station/customers', color: 'blue', showInNav: true, navOrder: 70, navGroup: 'Water Station' },
...
const WsDeliveryEntity: EntityDefinition<WsDelivery> = {
  name: 'ws_delivery',
  ui: { label: 'WS Delivery', labelPlural: 'WS Deliveries', icon: 'Truck', routePath: 'water-station/deliveries', color: 'green', showInNav: true, navOrder: 71, navGroup: 'Water Station' },
  sync: { enabled: true, conflictStrategy: 'lww', priority: 'normal' },
  audit: { enabled: true }, rbac: { enabled: true, permissionPrefix: 'water_station' },
  hooks: {}, pagination: 'cursor', tenant: { enabled: true }, softDelete: { enabled: true },
}
