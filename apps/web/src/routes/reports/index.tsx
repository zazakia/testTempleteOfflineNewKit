  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        setData({
          lookups, members: ml, loans: ll, payments: pl, accounts: al, entries: el,
...
function DelinquentReport({ d }: any) {
  ...
  return <Card><CardHeader title="Delinquent Accounts" description={`${dl.length} loan${dl.length !== 1 ? 's' : ''} past due`} />
    ...
    <tbody className="divide-y divide-gray-100">{dl.map((l: any) => <tr key={l.id}><td className="py-2 pr-4 font-mono text-sm">{l.loanNumber}</td><td className="py-2 pr-4">{d.lookups?.resolveBorrower(l.borrowerId) ?? l.borrowerId}</td><td className="py-2 pr-4 text-right">₱{(l.principalAmount ?? 0).toLocaleString()}</td><td className="py-2 pr-4 text-right text-red-600 font-medium">{l.dpd}d</td><td className="py-2 text-right">{LoanService.computeAgingBucket(l.dpd ?? 0)}</td></tr>)}</tbody></table>}</Card>
}
